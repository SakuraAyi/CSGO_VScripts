...
_snd <- [
    ...
    "weapons/11/minigun/minigun_wind_up.wav", 
    "weapons/11/minigun/minigun_fire.wav", 
    "weapons/11/minigun/minigun_wind_down.wav", 
...
function OnPostSpawn () 
{
    ...
    for ( ent = Entities.First(); ent != null; ent = Entities.Next( ent ) )
    {
        ...
        else if ( ent.GetName().find("sentry_aim", 0) != null )aim = ent;
        ...
        else if ( ent.GetName().find("sentry_aud", 0) != null ) _aud = ent;

        else if ( ent.GetName().find("sentry_bullet", 0) != null )_aud2 = ent;
...
deploy <- function () 
{
    ...
    if ( Owner.GetVelocity().z == 0 )
    {
        ...
        while ( fall )
        {
            downVector = pos + sentry_m.GetUpVector() * -2;
        ...
        foreach ( key in Player )
        {
            ...
            if ( data != null && data.IsValid() )
            {
    		    ...
    		    if ( _identity.GetTeam() == HostileCamp && Alive(_identity) )
    		    {
        		    ...
        		    if ( ( myOrigin - otherOrigin ).LengthSqr() < 589824 && tl == 1.0 )
        		    {
