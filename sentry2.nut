...
function OnPostSpawn () 
{
    ...
    DoEntFire( "!self", "RunScriptCode", "BindUser()", 0.03, self, self );
...
Exp <- function (entity,ori)
{
    ...
    exp.__KeyValueFromInt("iMagnitude", 600);
    exp.__KeyValueFromInt("iRadiusOverride", 200);
    exp.__KeyValueFromString("ignoredEntity", "Player*");
        ...
        foreach ( key, value in BOT )
        {
            ...
            if ( data[0] != null && data[0].IsValid() )
            {
    		    ...
    		    if ( Alive(_identity) )
    		    {
                    ...
                    local dist = ( myOrigin - otherOrigin ).LengthSqr();
        		    if ( dist < 1048576 && tl == 1.0 )
        		    {
                        //if (dist < 16384)
                            //continue;
