...
::Fade <- function (player, color, fadeio, hold, delay)
{
    local l_fade = Entities.CreateByClassname("env_fade");
    BaseVS(l_fade).SetKeyValue("spawnflags", 5);
    BaseVS(l_fade).SetKeyValue("duration", fadeio);
    BaseVS(l_fade).SetKeyValue("ReverseFadeDuration", fadeio);
    BaseVS(l_fade).SetKeyValue("holdtime", hold);
    BaseVS(l_fade).SetKeyValue("rendercolor", color);
    EntFireByHandle(l_fade, "Fade", "", delay, player, l_fade);
    EntFireByHandle(l_fade, "kill", "", delay, null, null);
};

::CreateHideWeaponGUI <- function ()
{
    local l_ui = Entities.CreateByClassname("game_ui");
    BaseVS(l_ui).SetKeyValue("spawnflags", 64);
    BaseVS(l_ui).SetKeyValue("FieldOfView", -1);
    return l_ui;
};

::CreateNvgEffect <- function ()
{
    local l_cc = Entities.CreateByClassname("color_correction");
    BaseVS(l_cc).SetKeyValue("filename", "materials/correction/nvg.raw");
    BaseVS(l_cc).SetKeyValue("spawnflags", 0);
    BaseVS(l_cc).SetKeyValue("maxweight", 1);
    BaseVS(l_cc).SetKeyValue("exclusive", 1);
    BaseVS(l_cc).SetKeyValue("minfalloff", -1);
    BaseVS(l_cc).SetKeyValue("maxfalloff", -1);
    BaseVS(l_cc).SetKeyValue("fadeInDuration", 2);
    BaseVS(l_cc).SetKeyValue("fadeOutDuration", 0.3);
    BaseVS(l_cc).SetKeyValue("StartDisabled", 1);
    return l_cc;
};