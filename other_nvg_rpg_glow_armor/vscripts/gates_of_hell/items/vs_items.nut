        ...
        if (ent.GetModelName().find("missile") > 0)
        {
            ...
            else
            {
                ...
                if (plr != null)
                {
                    ...
                    local TriPos2 = _start + rocket.GetLeftVector() * abs(mainScope.RPG().igniteDist*2);
                    local TriPos3 = _start + rocket.GetForwardVector() * mainScope.RPG().igniteDist;
                    local TriPos4 = _start + rocket.GetForwardVector() * abs(mainScope.RPG().igniteDist);
                    local TriPos5 = _start + rocket.GetUpVector() * mainScope.RPG().igniteDist;
                    local TriPos6 = _start + rocket.GetUpVector() * abs(mainScope.RPG().igniteDist);
                    //BaseVS(rocket).GetHitPosition(TriPos);
                    
                    local frac = TraceLine(_start, TriPos, null);
                    local frac2 = TraceLine(_start, TriPos2, null);
                    local frac3 = TraceLine(_start, TriPos3, null);
                    local frac4 = TraceLine(_start, TriPos4, null);
                    local frac5 = TraceLine(_start, TriPos5, null);
                    local frac6 = TraceLine(_start, TriPos6, null);
                    ...
                    if (pass)
                    {
                        ...
                        BaseVS(exp).SetKeyValue("classname", "prop_exploding_barrel");
                        BaseVS(exp).SetKeyValue("iMagnitude", mainScope.RPG().Magnitude);
                        BaseVS(exp).SetKeyValue("iRadiusOverride", mainScope.RPG().Radius);
                        BaseVS(exp).SetKeyValue("origin", _start);
                        exp.SetOwner(plr);
