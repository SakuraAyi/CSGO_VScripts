...
hudtxt <- [
    ["弹药量:","Ammo"],
    ["【炮塔菜单】最大弹容量:600","[Menu]FullAmmo:600"],
    ["补充弹药(7.62 x 100)","Reload(7.62 x 100)"],
    ["移动炮塔", "Move"],
    ["生命值：", "Helath:"]
];
glow <- null;
...
_snd <- [
    ...
    "11/weapons/minigun/minigun_wind_up.wav", 
    "11/weapons/minigun/minigun_fire.wav", 
    "11/weapons/minigun/minigun_wind_down.wav", 
    "Survival.DroneGunFocusPlayer", 
    "11/nazi/prop/explode3.mp3",
...
function OnPostSpawn () 
{
    ...
    for ( ent = Entities.First(); ent != null; ent = Entities.Next( ent ) )
    {
        ...
        if ( ent.GetName().find("sentry_but", 0) != null )
        {
        ...
        else if ( ent.GetName().find("sentry_gun", 0) != null )
        {
            ...
            if ( ent.GetMoveParent() == self )
            {
                ...
                gun = ent;
                gun.__KeyValueFromString("classname", "dronegun");

            }
        }

        else if ( ent.GetName().find("sentry_aim", 0) != null )
        {
        ...
        else if ( ent.GetName().find("sentry_cas", 0) != null )
        {
        ...
        else if ( ent.GetName().find("sentry_aud", 0) != null )
        {
        ...
        else if ( ent.GetName().find("sentry_bullet", 0) != null )
        {
        ...
        else if ( ent.GetName().find("sentry_hud", 0) != null )
        {
        ...
        else if ( ent.GetName().find("glow", 7) != null )
        {
            ...
            if ( ent.GetMoveParent() == self )
            {
                ...
                ent.__KeyValueFromString("targetname", "sentry_glow" + ne);
                glow = ent.GetName();
    ...
    //{
        ...
        local pos = gun.GetOrigin() + self.GetForwardVector() * -10;
    ...
    EntIO( but, "OnPressed", self.GetName(), "RunScriptCode", "OnUse()", "0.00", "-1" );
    EntIO( self, "OnTakeDamage", "!self", "RunScriptCode", "UnderAttack()", "0.00", "-1" );
...
BindUser <- function ()
{
    ...
    if ( Owner != null && Owner.IsValid() )
    {
        ...
        if ( self.GetOwner() == null || !self.GetOwner().IsValid() )
            gun.SetOwner(Owner);
        ...
        //{
            ...
            EntIO( ui, "PressedAttack", self.GetName(), "RunScriptCode", "deploy()", "0.00", "-1" );
...
deploy <- function () 
{
    ...
    if ( Owner.GetVelocity().z == 0 )
    {
        ...
        {
            pos.z += 32;
            downVector = pos + sentry_m.GetUpVector() * -34;
        ...
        EntFireByHandle(hud, "Enable", "", 0.00, null, null);
        ...
        if ( Owner == atvr )
        {
            if ( state == form.place )
            {
                if ( ui == null || ui != null && !ui.IsValid() )
                {
                    ...
                    EntIO(ui, "PressedForward", self.GetName(), "RunScriptCode", "select(0)", "0.00", "-1");
                    EntIO(ui, "PressedBack", self.GetName(), "RunScriptCode", "select(1)", "0.00", "-1");
                    EntIO(ui, "PressedAttack", self.GetName(), "RunScriptCode", "OnConf()", "0.00", "-1");
                    EntIO(ui, "PressedAttack2", "!self", "Deactivate", "", "0.00", "-1");
                    EntIO(ui, "PlayerOff", self.GetName(), "RunScriptCode", "MenuOff()", "0.00", "-1");
...
DisPmenu <- function ()
{
    if ( MenuShow )
    {
        ...
        if ( menu == null )
        {
            ...
            menu.__KeyValueFromInt("channel", 2);
...
OnConf <- function ()
{
    ...
    EntFire("clientcommand", "command", "Play " + _snd[6], 0.00, Owner);
    ...
    switch ( _activate )
    {
        ...
        foreach ( key, value in BOT )
        {
            ...
            if ( data[ 0 ] != null && data[ 0 ].IsValid() )
            {
    		    ...
    		    if ( _identity.GetTeam() == HostileCamp && Alive(_identity) )
    		    {
        		    ...
        		    local tlpos = gun.GetOrigin(),tlpos2 = otherOrigin;

        		    tl = TraceLine( tlpos, tlpos2, self );
        		    //GetHitPosition( self.GetOrigin(), otherOrigin );

        		    if ( ( myOrigin - otherOrigin ).LengthSqr() < 360000 && tl == 1.0 )
        		    {
        ...
        if ( _target != null && Alive(_target) )
        {
            ...
            if ( currentPitch != pitch )
            {
                ...
                if ( pitch > currentPitch )
                    ...
                    self.SetAngles( ( currentPitch += 2 ),0,0 );
		    ...
		    if ( /*( pitch - currentPitch <= 15 && pitch - currentPitch >= -15 ) && */( yaw - currentYaw <= 15 && yaw - currentYaw >= -15 ) )
            {
                ...
                if ( !_contact )
                {
                    ...
                    pos.z -= 32
                    pos += self.GetForwardVector() * 8;
                    DispatchParticleEffect( "extinguish_embers_small_01", pos, Vector(90.000000, currentYaw, 0.000000) );

                    EntFireByHandle( shell, "Start", "", 0.00, null, null );
                    DoEntFire( "!self", "SetAnimation", "firing", 0.00, self, self );

                    EntFireByHandle( _aud, "AddOutput", "message " + _snd[0], 0.00, null, null);
                    EntFireByHandle( _aud, "PlaySound", "", 0.00, null, null);
                    EntFireByHandle( _aud, "AddOutput", "message " + _snd[1], 0.03, null, null);
                    EntFireByHandle( _aud, "PlaySound", "", 0.03, null, null);
                ...
                pos = gun.GetOrigin() + gun.GetForwardVector() * 16;
                aim.SetOrigin( Vector( pos.x+r, pos.y+r2, pos.z+r3 ) );

                EntFireByHandle( fire, "Fire", "", 0.00, null, null );
                EntFireByHandle( gun, "Enable", "", 0.00, Owner, Owner );

                if ( second%33 == 0 )
                {

                    DispatchParticleEffect( "weapon_confetti_sparks", fire.GetOrigin(), Vector(90.000000,currentYaw,0.000000) );
                    second = 0;
                ...
                Ammo -= 1;
                UpdateText();
                EntFireByHandle( gun, "Disable", "", 0.01, null, null );
...
CeaseFire <- function ()
{
    ...
    if ( _contact )
    {
        ...
        EntFireByHandle( _aud, "AddOutput", "message " + _snd[2], 0.00, null, null);
        EntFireByHandle( _aud, "PlaySound", "", 0.00, null, null);
