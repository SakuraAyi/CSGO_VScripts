...
DestroyDummyAndRPG <- function ()
{
    ...
    BaseVS(Owner).SetKeyValue("disableflashlight", 0); BaseVS(Owner).SetKeyValue("disablereceiveshadows", 0); BaseVS(Owner).SetKeyValue("disableshadowdepth", 0); BaseVS(Owner).SetKeyValue("disableshadows", 0); BaseVS(Owner).SetRenderAmt(0, 255); BaseVS(Owner).SetPlayerSpeed(4, 1.0);
};
...
Think <- function ()
{
    ...
    view.__KeyValueFromVector("origin", _Vector);
